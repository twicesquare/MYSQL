...
ON a1.player_id = a2.player_id AND a2.event_date = DATE_ADD(a1.first_login, INTERVAL 1 DAY)
```
...
| 表达式 | 含义 | 匹配示例 |
| :--- | :--- | :--- |
| `[abc]` | 只能是 a 或 b 或 c | "a", "b" |
| `[0-9]` | 只能是一个数字 | "0", "5" |
| `[A-Za-z0-9]` | 任何字母或数字 | "A", "z", "9" |
| `[^0-9]` | 除了数字以外的任何字符 | "A", "!", "@" |

`@leetcode\\.com`：匹配固定的后缀。
...
```
## `UNION ALL`：结果集的“垂直叠加”
当你从不同的表或者用不同的逻辑查出了两组结果，想要把它们拼成一个长表输出时，就用 UNION ALL。

**核心规则**

列数必须相等：上面的查询查 2 列，下面也得查 2 列。

列顺序和类型要对应：第一列都是姓名，第二列都是日期。

不会去重：UNION ALL 会保留所有数据。**如果你想让合并后的结果自动删掉重复行，请改用 UNION（但 UNION ALL 性能更快）。**

## `LIKE`：模糊匹配
当你不知道确切的字符串，只记得其中一部分时，就用 LIKE 配合通配符。
...
&nbsp;&nbsp;&nbsp;&nbsp;`LIKE '%手机%'`：包含“手机”两个字的（华为手机、手机壳）。

`_`：代表一个字符。

&nbsp;&nbsp;&nbsp;&nbsp;`LIKE '张_'`：找姓张且名字只有一个字的（张三、张飞，但张长弓不行）。

## `LIMIT 1`
LIMIT 负责截取结果集的一部分，常用于分页或取最值。

只取第一行：即使查询结果有 1000 行，加上 `LIMIT 1` 数据库也只给你第一行。

配合 `ORDER BY` 才有意义：通常我们先用 `ORDER BY` 排序（比如按分数从高到低），然后用 `LIMIT 1` 就能锁死“冠军”
